...
// }
// else
// this.router.navigateByUrl(`/dashboard/candidate-details/${item.candidateId}${item.reqServiceRequest?.[0]?.requestId ? `?requestId=${item.reqServiceRequest?.[0]?.requestId}` : ''}`);
// const candidateId = item.candidateId;
// const requestId = item.reqServiceRequest?.[0]?.requestId;
// this.router.navigateByUrl(`/dashboard/candidate-details?id=${candidateId}&requestId=${requestId}`);
const candidateId = item.candidateId;
let requestId = item.selectedRequestId || '0';
if (requestId) {
this.router.navigateByUrl(`/dashboard/candidate-details/${candidateId}?requestId=${requestId}`);
}
}
}