...
export class CandidateScheduleComponent implements OnInit {
  ...
  selectCandidate(item: any): void {
    // console.log(item?.candidateId ,item['serviceSequence.serviceId'] ,"item")
    this.router.navigateByUrl(`/dashboard/candidate-details/${item?.candidateId}?requestId=${item['serviceSequence.serviceId']}`);
  }
